...
//
// 原实现允许通过 GODEBUG=tls10server=1 让服务端回退到 TLS 1.0 最低版本；
// fork 后不再提供该回退开关，客户端/服务端一致固定以 TLS 1.2 为最低版本。
func (c *Config) supportedVersions(isClient bool) []uint16 {
	versions := make([]uint16, 0, len(supportedVersions))
	for _, v := range supportedVersions {
		if fips140tlsRequired() && !slices.Contains(allowedSupportedVersionsFIPS, v) {
			continue
		}
...
// errNoCertificates 原实现通过 //go:linkname 暴露给第三方包
...
func isDisabledSignatureAlgorithm(version uint16, s SignatureScheme, isCert bool) bool {
	if fips140tlsRequired() && !slices.Contains(allowedSignatureAlgorithmsFIPS, s) {
		return true
	}
	...
	if version > VersionTLS12 {
		...
		// 原实现允许通过 GODEBUG=tlssha1=1 在 TLS 1.2 及以下重新启用
		// SHA-1 签名算法，fork 后不再提供该回退开关，固定禁用。
