...
// cipherSuitesTLS13 在上游 crypto/tls 中通过 //go:linkname 暴露给
// github.com/quic-go/quic-go 等第三方包直接访问；该机制按包导入路径
// 精确匹配（"crypto/tls".cipherSuitesTLS13），对 fork 后的
// github.com/cxio/wtls 无意义，故不再保留 //go:linkname 编译指令。
...
// hasAESGCMHardwareSupport 原本基于 internal/cpu 做逐平台硬件加速探测，
// 但 internal/cpu 是标准库私有包，fork 后的模块无法导入。
// 现代主流部署平台（amd64/arm64）普遍具备 AES-NI/ARMv8 Crypto Extensions，
// 这里简化为恒真值：仅影响 AES-GCM 密码套件的偏好排序（性能启发式），
// 不影响握手正确性。
var hasAESGCMHardwareSupport = true
...
func macSHA1(key []byte) hash.Hash {
	h := newConstantTimeHash(sha1.New)
...
// aeadAESGCM 原本经由 crypto/internal/fips140/aes/gcm.NewGCMForTLS12（FIPS
// 校验边界内实现）构造，fork 后改用公开的 crypto/cipher.NewGCM，
// 行为等价（标准 96 位 nonce 的 AES-GCM），仅不具备 FIPS 140 认证边界。
...
// aeadAESGCMTLS13 同上，改用公开的 crypto/cipher.NewGCM 替代
