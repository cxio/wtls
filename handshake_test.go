...
// bogo-mode 相关 flag 及 BoGo（BoringSSL）互操作测试套件（bogo_shim_test.go）
// 本轮暂不迁移，故不再保留这些 flag。
var (
...
func runMain(m *testing.M) int {
	...
	// 原实现在此额外构造 testConfigFIPS140（FIPS140-only 模式专用测试配置），
	// 仅被本轮未迁移的 fips140_test.go 使用；wTLS 不支持该模式，故不再保留。
