...
import (
	...
	tls "github.com/cxio/wtls"
)

// 注：上游 example_test.go 中依赖 net/http、net/http/httptest 的两个示例
// （ExampleConfig_keyLogWriter、ExampleX509KeyPair_httpServer）本轮未迁移。
// 原因：net/http.Server.TLSConfig 与 net/http/httptest.Server.TLS 字段的类型
// 硬编码为标准库 *crypto/tls.Config，wTLS 是独立的 fork（不同的导入路径/
// 类型），无法直接赋值给这两个字段，这是任何非 crypto/tls 本身的 fork
// 都存在的天然限制，并非本次迁移遗漏。
