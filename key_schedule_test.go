...
func TestACVPVectors(t *testing.T) {
	...
	es := NewEarlySecret(sha256.New, psk)
	...
	if got := es.EarlyExporterMasterSecret(transcript).secret; !bytes.Equal(got, earlyExporterMasterSecret) {
	...
	if got := ms.ExporterMasterSecret(transcript).secret; !bytes.Equal(got, exporterMasterSecret) {
