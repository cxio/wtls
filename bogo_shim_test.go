...
func TestBogoSuite(t *testing.T) {
	...
	if *bogoLocalDir != "" {
		...
		bogoDir = fetchBoringSSLModule(t, boringsslModVer)
	}
...
// a directory.
func fetchBoringSSLModule(t *testing.T, modVer string) string {
	t.Helper()

	tempDir := t.TempDir()
	bogoDir := filepath.Join(tempDir, "boringssl")

	testenv.MustHaveExecPath(t, "git")

	idx := strings.LastIndex(modVer, "-")
	if idx == -1 || idx == len(modVer)-1 {
		t.Fatalf("invalid modVer format: %q", modVer)
	}
	commitSHA := modVer[idx+1:]

	t.Logf("cloning boringssl@%s to %q", commitSHA, bogoDir)
	cloneCmd := testenv.Command(t, "git", "clone", "--no-checkout", "https://boringssl.googlesource.com/boringssl", bogoDir)
	if err := cloneCmd.Run(); err != nil {
		t.Fatalf("git clone failed: %v", err)
	}

	checkoutCmd := testenv.Command(t, "git", "checkout", commitSHA)
	checkoutCmd.Dir = bogoDir
	if err := checkoutCmd.Run(); err != nil {
		t.Fatalf("git checkout failed: %v", err)
	}

	t.Logf("fetched boringssl to %q", bogoDir)
	return bogoDir
}
