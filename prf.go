...
//
// 原实现经由 crypto/internal/fips140/tls12.PRF（FIPS 校验边界内实现）完成，
// fork 后改为直接调用本文件已有的公开 pHash 函数，两者算法完全一致：
...
//
// 原实现在 TLS 1.2 场景下改走 crypto/internal/fips140/tls12.MasterSecret（FIPS
// 校验边界内实现）；该函数与下方 prf(...) 分支使用的是完全相同的 PRF 公式
// （仅 label 固定为 "extended master secret"），fork 后统一走本地 prf 实现。
