...
func (c *Conn) loadSession(hello *clientHelloMsg) (
	...
	if session.version != VersionTLS13 {
		...
		if !session.extMasterSecret && fips140tlsRequired() {
			return nil, nil, nil, nil
		}
	...
	earlySecret = NewEarlySecret(cipherSuite.hash.New, session.secret)
...
// checkKeySize 原实现允许通过 GODEBUG=tlsmaxrsasize=<n> 覆盖 RSA 密钥
// 大小上限，fork 后不再提供该覆盖开关，固定使用 defaultMaxRSAKeySize。
